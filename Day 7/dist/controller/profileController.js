...
export const updateProfileController = async (req, res, next) => {
    ...
    try {
        ...
        if (imageFile) {
            ...
            const imageSize = (req.file?.size && Number((req.file?.size / 1024 ** 2).toFixed(2))) ||
                0;
            console.log(mimetype);
            console.log(req.file?.mimetype.split("/")[0]);
            if (!mimetype) {
                const err = new AppError("please upload image file *jpeg , jpg , png , gif ", 400);
                return next(err);
            }
            if (imageSize > 1) {
            ...
            const compressedPath = path.join(__dirname, "assets", "compressedImages", `${isUserExist.profilePicture ? isUserExist.profilePicture.split("/").at(-1) : `profile-${Date.now()}.jpg`}`);
